...
class Save extends \Magento\Backend\App\Action {
  ...
  public function __construct(Action\Context $context, PostDataProcessor $dataProcessor, Url $forumUrl, DateTime $date) {
  ...
  protected function _isAllowed() {
    return $this->_authorization->isAllowed('WIO_Forum:forum_save');
  }
    ...
    if ($data) {
      ...
      if (empty($data['url_text'])) {
        ...
        if ($this->modelUrl->notValidUrlKey($data['url_text'], $id)) {
          $errMessage = __('Invalid url text for forum "%1", its already exists', $data['url_text']);
          $urlKeyValid = false;
        }
      }
