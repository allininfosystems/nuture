...
class NewAction extends \Magento\Newsletter\Controller\Subscriber {
    ...
    Context $context, SubscriberFactory $subscriberFactory, Session $customerSession, StoreManagerInterface $storeManager, CustomerUrl $customerUrl, CustomerAccountManagement $customerAccountManagement
    ) {
        $this->customerAccountManagement = $customerAccountManagement;
        parent::__construct(
                $context, $subscriberFactory, $customerSession, $storeManager, $customerUrl
        );
    }
    ...
    protected function validateGuestSubscription() {
        if ($this->_objectManager->get('Magento\Framework\App\Config\ScopeConfigInterface')
                        ->getValue(
                                \Magento\Newsletter\Model\Subscriber::XML_PATH_ALLOW_GUEST_SUBSCRIBE_FLAG, \Magento\Store\Model\ScopeInterface::SCOPE_STORE
                        ) != 1 && !$this->_customerSession->isLoggedIn()
        ) {
            throw new \Magento\Framework\Exception\LocalizedException(
            __(
                    'Sorry, but the administrator denied subscription for guests. Please <a href="%1">register</a>.', $this->_customerUrl->getRegisterUrl()
            )
            );
        }
    }
    ...
    public function execute() {
        //echo "<pre>"; print_r($this->getRequest()->getParam('email')); echo "</pre>"; exit();
        if ($this->getRequest()->isPost() && $this->getRequest()->getParam('email')) {
            ...
            try {
                ...
                if ($status == \Magento\Newsletter\Model\Subscriber::STATUS_NOT_ACTIVE) {
                    ...
                    echo "The confirmation request has been sent.";
                } else {
                ...
                echo 'Something went wrong with the subscription.';
            }
        }
