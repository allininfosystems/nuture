...
namespace Webinse\Barcode\lib\Picqer\Barcode;
...
class BarcodeGeneratorJPG extends BarcodeGenerator
{
     ...
     * @param array $colorBg RGB (0-255) background color for bar elements.
     * @return string image data or false in case of error.
     * @public
     */
    public function getBarcode($code, $type, $totalHeight = 30, $color = array(0, 0, 0), $colorBg = array(255, 255, 255), $widthFactor = 2)
    {
        ...
        if (function_exists('imagecreate')) {
            ...
            $colorBackground = imagecolorallocate($jpg, $colorBg[0], $colorBg[1], $colorBg[2]);
        ...
        imagefilledrectangle($jpg, 0, $totalHeight - 10, $positionHorizontal, $totalHeight, $colorBackground);
        $font = dirname(__FILE__) . "/fonts/roboto.ttf";
        $box = imagettfbbox(8, 0, $font, join('   ', str_split($code)));
        $startTextPosition = ($positionHorizontal - $box[2]) / 2;
        imagettftext($jpg, 8, 0, $startTextPosition, $totalHeight - 1, $colorForeground, $font, join('   ', str_split($code)));
        ob_start();
        if ($imagick && isset($imageMagickObject)) {
